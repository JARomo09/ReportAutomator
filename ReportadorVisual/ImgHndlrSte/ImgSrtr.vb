...
Module ImgSrtr
    ...
    Sub Main()
        'Subroutine for navigating into a pre-existing location on desktop and organize images contained in it.
        'If the pre-defined location doesn't exist, proceeds to create it and prompt user to fill with images.
        'Outputs a fully formated word & PDF document, with all discriminated and sorted images from intput location.
        'Capable of outputing portrait & landscape formats, do check the mode and title variables "Mde", "Title" to select output format. 
        ...
        Dim SubLoc As String = Loc1 & "\Screenshots"
        Dim Mde As Integer = 2 '1 or 2: expenses report, 
        Dim Title As String = "Expenses Summary"
    ...
    Sub CreateReport(mode As Integer, title As String, location As String, location2 As String, Grp As List(Of String))
        ...
        With WdApp
            ...
            If Limit >= 1 Then 'Conditional to kill program in the event the number of screenshots is less than 1
                ...
                While Counter < Limit
                    ...
                    End If
                End While
                .ActiveDocument.SaveAs2(location & "\Initialization Summary" & ".docx", WdSaveFormat.wdFormatXMLDocument)
                .ActiveDocument.Paragraphs(1).Range.Delete()
                .ActiveDocument.Save()
                .ActiveDocument.ExportAsFixedFormat(location & "\Initialization Summary" & ".pdf", WdExportFormat.wdExportFormatPDF)
            Else : End If
            Beep()
        End With
        ...
        'UsableHeight = Nothing
        'UsableWidth = Nothing
        'UsableProportion = Nothing
        'TableX = Nothing
        'CycleNumb = Nothing
        'Counter = Nothing
        'Item = Nothing
    ...
    Sub InsrTbl(tabla As Integer, filas As Long, columnas As Long, FilEncabezado As String)
        'Originator: Javier Romo
        'https://www.linkedin.com/in/jaromo/
        'Summon to create a header table, consistent with corporate style standards
    ...
    Function ImgType(folder As String, mode As Integer) As List(Of String)
        ...
        Select Case mode
            Case 1 'Output 1 or 2: expenses report
                For Each Docu In DocuArr
                    ...
                    Else : End If
                Next
            Case 2 'Output 1 or 2: expenses report
                For Each Docu In DocuArr
                    If UCase(Docu.Extension) = ".PNG" Or UCase(Docu.Extension) = ".JPG" Or UCase(Docu.Extension) = ".JPEG" Or UCase(Docu.Extension) = ".TIFF" Then
                        SSList.Add(Docu.Name)
                    Else : End If
